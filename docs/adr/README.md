...
**Decision**: A **device pairing code exchanged for a long-lived, narrowly-scoped device token**. The plugin displays a code, the streamer approves it from an already-authenticated dashboard session and picks the overlay, and the plugin stores the resulting token. Deliberately framed around device tokens rather than Stream Deck, so Touch Portal and a stream-side CLI reuse it unchanged. A device token is bound to one user and one overlay, carries an explicit action scope (`engagement:write`, `chat:send`), is individually revocable, and has a use-refreshed sliding expiry. It is **not** a session: it does not satisfy the premium gate or the advanced-controls consent grant by itself, both of which are still evaluated against the owning streamer at call time, so a device cannot route around `requireEngagementPremium` or the `user:write:chat` re-consent. Rejected: pasting a JWT (expires in 24 h, full-power, breaks daily); loopback OAuth with PKCE (over-solves — the plugin needs All-Chat identity, not platform identity, and would mean loopback redirects registered with three providers); pasted personal access tokens (a secret this population would leak on camera); and a local bridge where the plugin drives the open dashboard tab (rejected for shipping because it dies when the tab closes, which is the babysitting the streamer asked to be rid of, but recommended as the shape of a throwaway spike)
