...
**Status**: ✅ Accepted
**Date**: 2026-08-23
**Problem**: All-Chat reacted to Twitch moderation only through its *effects* — `channel.chat.message_delete`, `channel.chat.clear_user_messages` and `channel.chat.clear` say a message vanished, a user's history was wiped, or chat was emptied, but carry **no acting moderator**, so a streamer could see that `someviewer` was timed out and had no way to learn which of their five mods did it, for how long, or why. Separately, a message AutoMod holds is never delivered as chat at all, so from All-Chat's point of view the viewer simply did not speak and the streamer has to sit in Twitch's own AutoMod queue — defeating the single-pane premise
