...
**Decision**: Preload the applied theme's full CSS into the editor so it's visible and directly editable; debounce-push edits to the preview as you type (brace-balance guarded, fonts proxied); persist a copy to `custom_css` only when the content diverges from the pristine theme ("fork-on-edit", `persistedCustomCss`) so untouched themed overlays stay linked and auto-update; add "Reset to theme" and surface Monaco CSS diagnostics as line-numbered tips. Pure logic in `lib/utils/custom-css.ts`, unit-tested
**Impact**: Streamers can see + edit theme CSS with a live preview; untouched overlays are unchanged on the wire and keep receiving theme fixes; only deliberately-edited overlays store a frozen copy. No new premium gate (enhances an existing free tool); CSP + `scopeCustomCss` blast-radius controls unchanged. (ADR numbering shared with caesar-deployment, so this is 0043)
