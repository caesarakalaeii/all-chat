...
#### YouTube leg, as built (amendment, 2026-08-12)

The plan below makes "completing timeout + delete + unban" a hard prerequisite for this leg. **Timeout shipped; delete and unban did not, and cannot on the terms assumed.** The prerequisite's purpose was moderation safety — not handing a volunteer permanent-ban-only — and timeout satisfies exactly that. The two omissions are blocked by missing IDENTIFIERS, not by missing work:

- **Single-message delete.** `liveChatMessages.delete` keys on a Data API message id (`LCC.`-prefixed). Production ingests chat through the **InnerTube** listener, whose renderer ids are a different encoding, so the id All-Chat holds for a YouTube message is not the id that endpoint accepts. The empirical list already flags this equivalence as unmeasured; until someone measures it, shipping delete would light a button that 404s. (It would work for the quota-based listener, which is not deployed — so it cannot be shipped on the strength of that either.)
- **Unban.** `liveChatBans.delete` keys on the ban resource id `insert` returns, which nothing persists and no list endpoint can recover. Adding persistence is possible but is a new table plus an unmeasured question — whether a ban id outlives the broadcast that created it — and it is not a prerequisite: a grant's default action set is `[delete, timeout]`, and a temporary ban expires by itself.

So YouTube's capability is **timeout + ban**, and `PlatformActions["youtube"]` says so. A default grant on a YouTube source therefore delegates timeout, which is the proportionate tool.

**The anchor gates the OWNER path here, and YouTube is the reason that rule exists.** `Resolve` falls back to the channel-agnostic `users` row of a YouTube-login account, which matches any channel id — so without the anchor an owner could aim moderation at a channel they merely added as a read-only source. The platform would refuse it, but All-Chat would have asked, and the anchor rule is about not asking. `OwnerYouTubeAnchor` accepts **only** the per-channel `youtube_oauth_tokens` row (Google's own attestation: it exists because Google issued a token for that channel's account) and returns no id, since a YouTube write is addressed by the broadcast's liveChatId. Re-measured immediately before enabling, as the decision table requires: **406 of 406** production YouTube sources are anchored, so no existing streamer is affected.

**Resolved from the empirical list: mod consent needs no `channels.list?mine=true` at all**, so the force-ssl-only question is moot. The shared callback resolves channel info only for add-source states, and a mod-consent state is not one; the credential is attributed by the Google account id from userinfo. Consent therefore requests force-ssl + `userinfo.profile` and **not** `youtube.readonly` — that scope exists so youtube-listener can poll a streamer's own chat, and asking a volunteer for it would be an ADR-0012 regression for a capability the moderation path never uses.

**One defect this leg surfaced, worth recording because it defeated `errors.Is` silently:** the owner-reach sentinel existed twice, once in `moderation-service/tokens` and once in the new shared YouTube anchor. Two `errors.New` values with the same text compare unequal, so the dispatcher would have reported an unanchored owner as a 502 platform error rather than the actionable `owner_channel_unverified`. The shared package now owns the sentinel and moderation-service aliases it, exactly as it already aliases `ErrNoCredential`. A test caught this, not review.
...
#### Kick leg, as built (amendment, 2026-08-12)

Three things about Kick that the plan above did not anticipate.

**The anchor is load-bearing in a way it is not on Twitch.** Kick's moderation endpoints carry no moderator field — the acting identity is implied by the bearer token — so `broadcaster_user_id` is the *only* id in the request. On Twitch, sourcing the broadcaster id from the acting user's own credential produces a call Helix refuses; on Kick it produces a call Kick *accepts*, against the moderator's own channel. The failure mode is silent and wrong rather than loud, which is why `OwnerKickAnchor` is the single source of that id and gates every delegated Kick action — including delete, where the value goes unused because Kick addresses the message directly, but where the question ("does the owner control this channel at all?") is exactly as necessary.

**The 401-vs-403 unknown is handled rather than resolved.** The empirical list below wants Kick's mapping measured with two staging accounts, which has not happened. What made the leg shippable anyway is that the ambiguity only has one dangerous branch: if Kick answers 401 where we expect 403, a volunteer is told to re-consent forever for a channel they were never modded on. So a 401 that survives a **successful** refresh is treated as `not_moderator_on_platform` on the delegated path — after a fresh token and a passed scope pre-check, credential validity is no longer a live explanation. The owner path keeps the re-consent answer, because a broadcaster is always a moderator of their own channel. If the measurement later shows Kick uses 403 as expected, this branch simply stops being reached.

**A prerequisite bug had to be fixed first, and it was not delegation-specific.** `KickOAuth.ExchangeCodeWithPKCE` parsed the token response's `scope` field and dropped it, so `ExtractGrantedScopes` returned nothing and **every** Kick grant — owner grants included — was stored with an empty `granted_scopes`. The consent flow appeared to succeed while `GET /capabilities` kept reporting `missing_scope` and the dispatcher's pre-check refused every Kick action, with no downstream repair possible (a refresh grant never widens scopes). Kick's flow is hand-rolled rather than going through `oauth2.Config.Exchange`, which would have populated `Extra` itself — the same shape of gap as the `@me` bug on Discord: a platform-specific path that never exercised the assumption the shared code makes.

Deliberately NOT done: fabricating `granted_scopes` from the *requested* scopes when Kick's response omits them. That field records what the platform confirmed, and a missing confirmation must read as `missing_scope`, not as a grant.
...
Empirical verification is required before the corresponding leg opens, and none of it is documented by the platforms: whether Twitch issues a token for moderation scopes alone; Kick's 401-vs-403 mapping and whether a moderator's own token reaches a channel they do not own *(still unmeasured as of 2026-08-12; the Kick leg shipped with the 401 branch handled defensively instead — see the Kick amendment above. The first real delegated Kick action is the measurement)*; whether YouTube's `channels.list?mine=true` works with force-ssl alone *(**moot as of 2026-08-12**: mod consent never calls it — see the YouTube amendment above)*, whether InnerTube message ids are accepted by `liveChatMessages.delete` *(**still unmeasured, and now load-bearing**: it is the sole reason YouTube delete is unbuilt)*, the real per-call quota cost of every `liveChat*` write (the existing "official" cost constant's provenance is wrong — the misleading word is gone as of 2026-08-12, the numbers are still estimates), and whether unban survives the broadcast that created the ban *(still unmeasured; YouTube unban stays unbuilt)*.
