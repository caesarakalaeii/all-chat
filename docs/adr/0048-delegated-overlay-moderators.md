...
| Kick single-message delete | **Its own PR, before** the delegation Kick leg, once the staging matrix confirms the endpoint and the 401-vs-403 mapping. It is an owner-facing capability correction, so it should not be coupled to a feature that might be rolled back. |
