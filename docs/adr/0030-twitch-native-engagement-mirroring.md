...
- **Negative / trade-offs**: In the rare both-live state, the simultaneously-live *native* round is hidden on the overlay until the All-Chat round ends (display prefers the points-bearing All-Chat round); this is non-destructive and the create-side 409 keeps it rare. Because the display precedence orders live rounds ahead of terminal ones, a native round going ACTIVE at the same moment an All-Chat round resolves/cancels can win the display during that round's grace window and pre-empt its "who won" reveal frame; this is an accepted trade-off of the live-outranks-terminal precedence — display-only (points are already credited) and kept uncommon by the create-side 409. Mirrored tallies refresh only as fast as Twitch sends `progress` events. The 7 extra subscriptions per opted-in channel add to the Twitch subscription budget (uncosted in-code today, same as the existing 8 types).
