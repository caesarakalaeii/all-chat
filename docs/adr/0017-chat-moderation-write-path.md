...
- **Per-platform**: Twitch full; Kick timeout/ban (new OAuth-2.1 client); Discord delete-only (new bot REST token); YouTube ban-only and feature-gated (re-adds `youtube.force-ssl`); TikTok unsupported (no API — reported `unsupported_platform`, never a fake button).
