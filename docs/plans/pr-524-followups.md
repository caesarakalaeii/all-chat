...
1. **Run migration 071** (`migrations/071_engagement_dedup_scope_and_announce.sql`) via the normal runner (globs `migrations/`, in-pod path). It: rescopes the two `source_message_id` replay-dedup indexes to per-round, adds a `LOWER(channel_id)` functional index, and adds `points_earn_config.announce_on_start`. **Not** mirrored into `migrations/init/` (frontend-dev doesn't run engagement) — intentional.
...
- **Integration regression tests are local-only** (the nightly matrix runs `go test ./...` without `-tags=integration` and has no Postgres). Run against a DB with migrations 067–071 applied:
...
- **Migration 071 against a real DB** — confirm the drop+recreate indexes and the functional index apply cleanly.
...
3. **[low]** migration 071 down-path recreated a *global unique* `source_message_id` index that would abort on legit multi-overlay rows. Fixed: down path recreates them **non-unique** (documented that global uniqueness can't be restored).
