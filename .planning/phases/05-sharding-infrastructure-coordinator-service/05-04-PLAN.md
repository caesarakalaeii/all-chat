...
Purpose: Enables Phase 6 listener integration by providing assignment query API (SHARD-03) and heartbeat publishing endpoint. Metrics expose coordinator state for monitoring and debugging.
...
<task type="auto">
  ...
  <done>
    handlers/assignments.go implements GetAssignments (SHARD-03) and PublishHeartbeat endpoints with Prometheus metrics integration. models/assignment.go updated with JSON tags for API responses.
  </done>
</task>
...
<task type="auto">
  ...
  <done>
    cmd/main.go initializes ShardMetrics, creates AssignmentHandler, registers /assignments (SHARD-03) and /heartbeat endpoints under protected route group, and passes metrics to coordinator. Service compiles successfully.
  </done>
</task>
...
- HTTP endpoints created (GET /assignments implements SHARD-03, POST /heartbeat)
