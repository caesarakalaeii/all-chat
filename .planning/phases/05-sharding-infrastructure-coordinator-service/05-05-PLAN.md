...
Purpose: Proves Phase 5 success criteria #5 ("Coordinator survives chaos testing") by simulating production failure modes and validating recovery within SLA (60 seconds for channel redistribution). Validates all Phase 5 requirements (SHARD-01 through SHARD-08, REBAL-08) under failure conditions.
...
<task type="auto">
  ...
  <action>
    ...
    **References:**
    ...
    - CONTEXT.md: Heartbeat timeout 15s (not 30s - user constraint for fast recovery)
    ```
  </action>
  <verify>
    ```bash
  ...
  <done>
    docs/testing/chaos-testing-phase5.md created with 5 chaos scenarios (leader failure, network partition, listener failure, simultaneous failures, Redis latency), validation procedures, and failure indicators. References user constraint (15s heartbeat timeout from CONTEXT.md).
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    **Note:** Some scenarios require manual execution (network partition, Redis latency) due to requiring privileged access or specific tooling. Script automates the testable subset and references 15s heartbeat timeout from CONTEXT.md.
  </action>
  <verify>
    ```bash
  ...
  <done>
    scripts/chaos-test-phase5.sh created with automated execution for Scenarios 1, 3, 4. Script validates leader election timing, assignment recovery, and cascading failure resilience. References 15s heartbeat timeout from user constraint.
  </done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <name>Task 3: Verify chaos testing suite</name>
  <what-built>
    ...
    - Validates all Phase 5 requirements (SHARD-01 through SHARD-08, REBAL-08) under failure conditions
  </what-built>
  <how-to-verify>
    **Prerequisites:**
    ...
    **Acceptance criteria:**
    ...
    - 15s heartbeat timeout user constraint (from CONTEXT.md) correctly referenced
  </how-to-verify>
  <resume-signal>
    Type "approved" if:
    ...
    - User constraint (15s heartbeat timeout) correctly applied
...
- [ ] Scenario 3 (listener failure) validates 15s heartbeat timeout detection (user constraint from CONTEXT.md) and redistribution
...
- [ ] All Phase 5 requirements (SHARD-01 through SHARD-08, REBAL-08) validated under failure conditions
</success_criteria>
...
- User constraint honored (15s heartbeat timeout from CONTEXT.md for fast recovery)
- Requirements validated (SHARD-01 through SHARD-08, REBAL-08)
