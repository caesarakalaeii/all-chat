...
must_haves:
  truths:
    ...
    - "DiscordRepository can UpsertGuild, DeleteGuild, ListGuildsByUser, and DeleteDiscordSourcesByGuildID against the discord_guilds table"
  artifacts:
    ...
    - path: "services/auth-service/repository/discord_repo.go"
      provides: "DiscordRepository with UpsertGuild, DeleteGuild, ListGuildsByUser, GetGuild, DeleteDiscordSourcesByGuildID"
  ...
  key_links:
    ...
    - from: "services/auth-service/repository/discord_repo.go"
      ...
      via: "DeleteDiscordSourcesByGuildID hard-deletes Discord sources on guild disconnect"
      pattern: "overlay_chat_sources"
---
...
Output: DiscordOAuth struct, DiscordRepository (including DeleteDiscordSourcesByGuildID), DiscordGuild model, and unit tests covering all four requirements.
</objective>
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Test: DeleteDiscordSourcesByGuildID deletes overlay_chat_sources where platform='discord' and config->>'guild_id' matches
  </behavior>
  <action>
...
Repository unit tests against a live DB are integration tests — they require the migration from Plan 01. The automated verify here confirms compile success; full integration testing happens during `/gsd:verify-work`.
  ...
  <done>DiscordGuild model and DiscordRepository compile without errors; GuildID is typed as string; DeleteDiscordSourcesByGuildID is present in discord_repo.go</done>
</task>
...
- `grep "DeleteDiscordSourcesByGuildID" services/auth-service/repository/discord_repo.go` — method present
</verification>
...
- DiscordRepository has UpsertGuild, DeleteGuild, ListGuildsByUser, GetGuild, DeleteDiscordSourcesByGuildID
