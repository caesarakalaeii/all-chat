...
Purpose: This is the service that will handle all Discord Gateway events in later phases. Phase 27 establishes the stable connection foundation — no message processing yet, just connection management. The Gateway connection is foundational infrastructure for all AUTH-01 through AUTH-04 requirements: without a working Gateway, the bot cannot receive GUILD_CREATE/GUILD_DELETE events, and channel listing and disconnect flows have no live bot session to operate against.
