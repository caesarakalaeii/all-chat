...
// Package status mirrors kick-listener's platform status publisher: the
// producer contract ADR-0032's client self-heal depends on.
package status
...
// NewPublisher creates a status publisher.
...
// Publish marshals and publishes a status message, logging failures at Warn.
