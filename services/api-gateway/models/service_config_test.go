...
func TestNewServiceRegistry(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantErr: false,
			checkFunc: func(t *testing.T, sr *ServiceRegistry) {
				assert.Len(t, sr.Services, 27) // 4 base + 7 admin + 9 share-service + 2 maintenance routes + 1 test-stream + 1 moderation + 2 payment + 1 engagement
				...
				assert.NotNil(t, sr.Services["share-service-ambassadors"])
				assert.NotNil(t, sr.Services["share-service-admin-ambassadors"])
				assert.NotNil(t, sr.Services["share-service-localization"])
				assert.NotNil(t, sr.Services["share-service-admin-localization"])
