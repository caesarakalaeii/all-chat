...
type WebSocketClient interface {
	...
	Unsubscribe(chatID string) error
	IsConnected() bool
	GetSocketID() string
}
...
func (m *Manager) syncChannels() error {
	...
	for slug, ch := range m.subscriptions {
		if _, exists := desiredChannels[slug]; !exists {
			...
			if err := m.wsClient.Unsubscribe(strconv.Itoa(ch.ChatroomID)); err != nil {
	...
	for slug, desired := range desiredChannels {
		...
		// Rumble chat is read-only and anonymous for arbitrary channels
		// (ADR-0061): no auth token, just open the SSE stream for the id.
		if err := m.wsClient.Subscribe(strconv.Itoa(desired.ChatroomID)); err != nil {
...
// fetchChatroomID resolves the chat room id for a channel. Rumble sources
// are stored as the numeric chat pop-up id, so "resolution" is validation:
// the value must already be numeric. No channel API is involved (ADR-0061).
func (m *Manager) fetchChatroomID(channelSlug string) (int, error) {
	channelID, err := strconv.Atoi(channelSlug)
	if err != nil || channelID <= 0 {
		return 0, fmt.Errorf("rumble channel %q is not a numeric chat id", channelSlug)
	}
