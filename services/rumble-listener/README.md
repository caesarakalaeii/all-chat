...
go build -o rumble-listener ./services/rumble-listener/cmd/
...
```bash
# Run tests
go test -v ./services/rumble-listener/...

# With coverage
go test -cover ./services/rumble-listener/...
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: rumble-listener
        image: ghcr.io/caesarakalaeii/allchat-rumble-listener:main
        ports:
        - containerPort: 8089
        env:
        - name: DATABASE_HOST
...
```bash
# View logs
kubectl logs -n allchat deployment/rumble-listener

# Follow logs
kubectl logs -n allchat deployment/rumble-listener -f

# Filter for errors
kubectl logs -n allchat deployment/rumble-listener | grep ERROR
```
...
- Check logs: `kubectl logs -n allchat deployment/rumble-listener`
- View status: `curl http://rumble-listener:8089/status`
