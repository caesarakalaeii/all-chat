...
// PatreonToken is a decrypted Patreon OAuth credential for one all-chat user.
...
// Upsert stores (or replaces) the Patreon connection for a user. Re-connecting the
// same user updates the row; linking a Patreon account already tied to a different
// user violates the patreon_user_id unique constraint and returns an error.
func (r *TokenRepository) Upsert(ctx context.Context, t PatreonToken) error {
	...
	_, err = r.db.Exec(ctx, `
		...
		ON CONFLICT (user_id) DO UPDATE SET
		    ...
		    updated_at         = NOW()`,
		t.UserID, t.PatreonUserID, encAccess, encRefresh, t.ExpiresAt, t.Scopes, int(r.currentKid()))
	if err != nil {
...
// UpdateTokens stores refreshed tokens for a user. A blank refresh token (some
// refresh responses omit it) leaves the stored refresh token untouched.
func (r *TokenRepository) UpdateTokens(ctx context.Context, userID string, token *oauth2.Token) error {
	...
	if token.RefreshToken == "" {
		_, err = r.db.Exec(ctx, `
			...
			WHERE user_id = $1`,
			userID, encAccess, token.Expiry, int(r.currentKid()))
	} else {
		...
		_, err = r.db.Exec(ctx, `
			...
			WHERE user_id = $1`,
			userID, encAccess, encRefresh, token.Expiry, int(r.currentKid()))
	}
	if err != nil {
...
// GetByPatreonUserID resolves the all-chat user id linked to a Patreon user id.
func (r *TokenRepository) GetByPatreonUserID(ctx context.Context, patreonUserID string) (userID string, found bool, err error) {
	err = r.db.QueryRow(ctx,
		"SELECT user_id FROM patreon_oauth_tokens WHERE patreon_user_id = $1", patreonUserID).Scan(&userID)
	if errors.Is(err, pgx.ErrNoRows) {
		return "", false, nil
	}
	if err != nil {
		return "", false, fmt.Errorf("lookup patreon token by patreon user id: %w", err)
	}
	return userID, true, nil
}

// Delete removes a user's Patreon connection (on disconnect).
func (r *TokenRepository) Delete(ctx context.Context, userID string) error {
	_, err := r.db.Exec(ctx, "DELETE FROM patreon_oauth_tokens WHERE user_id = $1", userID)
	if err != nil {
...
// oldest-updated first so repeated passes cover everyone.
func (r *TokenRepository) ListAll(ctx context.Context, limit int) ([]PatreonToken, error) {
	rows, err := r.db.Query(ctx, `
		SELECT user_id, patreon_user_id, access_token, refresh_token, token_expires_at, granted_scopes
	...
	for rows.Next() {
		...
		if err := rows.Scan(&t.UserID, &t.PatreonUserID, &encAccess, &encRefresh, &t.ExpiresAt, &t.Scopes); err != nil {
			r.logger.Warn("Failed to scan patreon token row", zap.Error(err))
			continue
		}
		if t.AccessToken, err = r.decrypt(encAccess); err != nil {
			r.logger.Warn("Failed to decrypt patreon access token", zap.String("user_id", t.UserID), zap.Error(err))
			continue
		}
		if t.RefreshToken, err = r.decrypt(encRefresh); err != nil {
			r.logger.Warn("Failed to decrypt patreon refresh token", zap.String("user_id", t.UserID), zap.Error(err))
			continue
		}
		out = append(out, t)
	}
