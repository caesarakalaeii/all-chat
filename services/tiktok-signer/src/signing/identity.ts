...
/**
 * The identities the signer presents to TikTok. One module because TikTok
 * silently empty-200s any request whose User-Agent disagrees with the URL's
 * self-describing params (browser_platform/browser_version/os) — measured
 * 2026-09-18, both version drift and a platform swap are rejected — so the
 * UA and the params it describes must come from the same object everywhere:
 * the signing session's page, imFetchParams, and GET /v1/identity.
 *
 * Two identities exist and both are pinned: the Safari/macOS session that
 * signs URLs with the in-page SDK, and the Linux Chrome/144 viewer whose
 * page captures and WS sessions the listener rides.
 */

/** Stable identity the signer presents to TikTok. Exposed via GET /v1/identity. */
...
export const SIGNING_IDENTITY: SignerIdentity = {
...
export const VIEWER_IDENTITY: SignerIdentity = {
  userAgent:
 ...
 */
export function identityIsConsistent(identity: SignerIdentity): boolean {
  const { userAgent, browserPlatform, os, screenWidth, screenHeight } = identity;
  const osSection = (userAgent.split(';')[1] ?? '').toLowerCase();
  if (!osSection.includes(os.toLowerCase())) return false;
  ...
  if (impliedOs === undefined) return false;
  if (!osSection.includes(impliedOs)) return false;
 ...
 */
export function browserVersionFromUserAgent(ua: string): string {
