...
| `GET /v1/session` | Shared WS session lease (pure-Node transport, PR 1 of the 2026-09-18 plan): the warm classic room's webcast push WS URL + freshly re-read cookie jar, `{ wsUrl, cookieHeader, roomId, userAgent, proxyHost, capturedAt }`. The listener consumes this in `pure-node` signer mode (`TIKTOK_SIGNER_MODE=pure-node`, `src/sign/pure-node.ts`): it synthesizes the connector's initial fetch result — forwarding the wsUrl's recorded identity params (handshake-load-bearing), cursor `0`, the cookie jar — and enters the target room cross-room. No capture cadence — a capture runs only on request (TikTok request-budget discipline). Flow: warm lease with a non-empty `wsUrl` is served; otherwise warm rooms are liveness-checked (TikTok's api-live user route, `user.status === 2`; a probe error fails open) and the first tab-less, live, breaker-eligible room is cold-captured — an offline warm room is skipped in milliseconds instead of burning its ~90s capture timeout; a recovery pass closes unservable tabs (skipping rooms with live relay subscribers) and cold-captures exactly the freed rooms; otherwise retryable 503 `no_warm_session`. |
...
| `SIGNER_WARM_ROOMS` | empty | Comma-separated classic rooms (lowercased) whose captured WS sessions `GET /v1/session` leases; empty disables the endpoint. **Seed list**: with `SIGNER_WARM_AUTOCURATE=on` the curator adds capture-verified rooms on its own when live coverage drops (see that row); without it this list is the whole truth and must list classic, verified-live rooms only — only classic rooms emit the harvestable im/fetch (the page variant is per-room; the curator's capture IS the classic-ness test). Warm tabs are pinned against idle eviction, and the capture breaker paces a dead or live_new room's retries. A room listed both here and in `SIGNER_RELAY_CANARY_ROOMS` is a misconfiguration whose canary jar would be leased as the primary session — the service refuses to start. |
| `SIGNER_WARM_AUTOCURATE` | `off` | `on` runs the warm-room auto-curator (`src/signing/warm-curator.ts`): every 30 min it probes tab-less warm rooms' liveness (api-live user route, fail-open); when live coverage (warm tabs + live rooms) falls below 2 it runs ONE discovery pass on the public live feed (a pool-lane page, same UA and blocking as a capture), filters candidates (never canary, never listed, 6h cooldown after a failed verification), liveness-checks them, and capture-verifies at most 2 — a passing capture both proves the room classic and leaves it warm. Any 403 (discovery or capture) benches all acquisition for an hour — the manual budget rule, encoded. The curator only ever ADDS rooms; removal stays a human call (breaker + `TikTokPureNodeNoWarmSession` alert cover failing rooms). Restart re-seeds from `SIGNER_WARM_ROOMS` and re-curates within minutes. |
