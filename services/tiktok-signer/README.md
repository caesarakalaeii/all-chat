...
| `SIGNER_PROXY_HOSTS` | empty | Comma-separated residential proxy list (`host:port,...`) for the viewer lanes: one browser per proxy, rooms pinned to their lane, failing lanes benched for a cooldown. **Required for page mode in production**: TikTok gates the chat bootstrap on IP reputation; the datacenter IP never receives im/fetch. Falls back to the singular `SIGNER_PROXY_HOST` when unset. |
