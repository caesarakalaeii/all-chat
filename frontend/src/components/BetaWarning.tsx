...
export function BetaWarning({ platform, onCancel, onContinue }: BetaWarningProps) {
  ...
  return (
    <Dialog.Root open onOpenChange={(open: boolean) => { if (!open) onCancel() }}>
      <Dialog.Content showCloseButton={false}>
        ...
        <a
          ...
          className="mt-2 inline-flex items-center gap-2 text-sm text-blue-400 hover:text-blue-300 underline underline-offset-4"
        >
        ...
        <div className="mt-6 flex gap-3 justify-end">
          <button
            onClick={onCancel}
            className="px-4 py-2 rounded-lg bg-surface-2 hover:opacity-80 text-text font-medium transition-opacity"
          >
            Cancel
          </button>
          <button
            onClick={onContinue}
            className="px-4 py-2 rounded-lg bg-yellow-600 hover:opacity-80 text-white font-medium transition-opacity"
          >
