...
function PlatformBadge({ platform }: { platform: string }) {
  ...
  return (
    <span
      className={cn(
        'inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium text-black',
        found.color
      )}
    >
  ...
  return (
    <div className={cn('lanes-app min-h-screen', archivoBlack.variable, spaceMono.variable)}>
      ...
      <main id="main-content" tabIndex={-1} className="mx-auto max-w-2xl space-y-6 px-4 py-12">
        <h1 className="text-2xl">{t('settings.viewer.heading')}</h1>
        <p className="text-sub text-sm">{t('settings.viewer.subheading')}</p>

        <div className="lanes-panel p-6">
          <h2 className="mb-2 text-lg">{t('settings.viewer.signInHeading')}</h2>
          <p className="text-sub mb-6 text-sm">{t('settings.viewer.signInBody')}</p>
          <div className="flex flex-col gap-3 sm:flex-row">
            <button onClick={() => viewerLogin('twitch')} className="lanes-btn gap-2.5 px-6 py-3">
              <svg
...
function ColorGradientCard({ claims }: { claims: ViewerJWTClaims }) {
  ...
  return (
    <div className="lanes-panel p-6">
      <h2 className="mb-1 text-lg">{t('settings.viewer.nameColorHeading')}</h2>
      <p className="text-sub mb-4 text-sm">{t('settings.viewer.nameColorBody')}</p>
        ...
        <div>
          <div className="flex items-center gap-3">
            <input
              ...
              className="h-10 w-10 cursor-pointer rounded-none border border-white/50 bg-transparent"
          ...
          <p className="text-sub mt-2 text-xs">{t('settings.viewer.autoSaveNote')}</p>

          {/* Live preview */}
          <div className="mt-4 border-t border-white/20 pt-4">
            <p className="text-sub mb-2 text-xs">{t('settings.viewer.previewLabel')}</p>
            <div className="flex items-center gap-2">
              <div className="text-sub flex h-6 w-6 items-center justify-center rounded-full border border-white/30 text-xs font-medium">
              ...
              <span className="text-sub text-sm">{t('settings.viewer.previewMessage')}</span>
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'gradient' && (
        <div>
            ...
            {gradientStops.map((stop, i) => (
              <div key={i} className="mb-2 flex items-center gap-2">
                <input
                  ...
                  className="h-8 w-8 cursor-pointer rounded-none border border-white/50"
                />
                <Input
                  type="text"
                  value={stop}
                ...
                {gradientStops.length > 2 && (
                  <button
                    ...
                    className="lanes-btn ghost px-2 py-1 text-lg leading-none"
          ...
          <div className="mt-3 flex items-center gap-3">
            <label htmlFor={angleId} className="text-sub w-10 text-xs">
          ...
          <div className="mt-4 flex items-center gap-3">
            <button className="lanes-btn" onClick={handleSaveGradient} disabled={gradientSaving}>
              {gradientSaving
...
function AvatarCosmeticsCard({ claims }: { claims: ViewerJWTClaims }) {
  ...
  return (
    <div className="lanes-panel p-6">
      <h2 className="mb-1 text-lg">{t('settings.viewer.cosmeticsHeading')}</h2>
      <p className="text-sub mb-4 text-sm">{t('settings.viewer.cosmeticsBody')}</p>
      ...
      <div className="mb-6">
        <h3 className="mb-3 text-sm font-medium">{t('settings.viewer.frameHeading')}</h3>
          ...
          {frames.map((item) => (
            <button
              ...
              className={cn(
                ...
                'relative rounded-none border-2 p-1 transition-colors',
                'focus-visible:ring-3 focus-visible:ring-ring/50 focus-visible:outline-none',
                selectedFrameId === item.id
                  ? 'border-twitch bg-black'
            ...
            >
              {item.image_url ? (
                ...
                <img
                  src={item.image_url}
                  alt={item.name}
                  className="h-16 w-16 rounded-none object-contain"
                />
              ) : (
                <div className="text-sub flex h-16 w-16 items-center justify-center rounded-none border border-white/30 text-xs">
                  {t('settings.viewer.noneItem')}
                </div>
              )}
              <span className="text-sub mt-1 block truncate text-center text-xs">{item.name}</span>
      ...
      <div className="mb-6">
        <h3 className="mb-3 text-sm font-medium">{t('settings.viewer.flairHeading')}</h3>
        <div className="grid grid-cols-4 gap-2">
          {flairs.map((item) => (
            <button
...
function LinkedPlatformsCard({ claims }: { claims: ViewerJWTClaims }) {
  ...
  return (
    <div className="lanes-panel p-6">
      <h2 className="mb-1 text-lg">{t('settings.viewer.linkedHeading')}</h2>
      <p className="text-sub mb-4 text-sm">{t('settings.viewer.linkedBody')}</p>
      ...
      <div className="flex flex-col gap-3">
        {PLATFORMS.map(({ key, color }) => {
          ...
          return (
            <div
              key={key}
              className="flex items-center justify-between rounded-none border border-white/40 px-4 py-3"
            >
              <div className="flex items-center gap-3">
                <span className={cn('h-3 w-3 rounded-full', color)} aria-hidden="true" />
                <span className="text-sm">{t(`common.platforms.${key}`)}</span>
              </div>
              {isConnected ? (
                <div className="flex items-center gap-2">
  ...
  return (
    <div className={cn('lanes-app min-h-screen', archivoBlack.variable, spaceMono.variable)}>
      ...
      <main id="main-content" tabIndex={-1} className="mx-auto max-w-2xl space-y-6 px-4 py-12">
        ...
        <div>
          <h1 className="text-2xl">{t('settings.viewer.heading')}</h1>
          <p className="text-sub mt-1 text-sm">{t('settings.viewer.subheading')}</p>
        </div>

        {/* Profile summary */}
        <div className="lanes-panel p-6">
          <h2 className="mb-4 text-lg">{t('settings.viewer.profileHeading')}</h2>
          <div className="flex items-center gap-3">
            {claims.avatar_url && (
              // eslint-disable-next-line @next/next/no-img-element
              <img
